...
The resulting directory structure is now split into train and test sets:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--test-split-ratio", type=float, default=0.3, help="Ratio of samples for the test set (e.g., 0.3 for 30%).")
...
def main() -> None:
    ...
    print(f"\nInitial dataset created at: {out_root}")
    ...
    if has_fft:
        (train_dir / "ffts").mkdir(parents=True, exist_ok=True)
        (test_dir / "ffts").mkdir(parents=True, exist_ok=True)

    all_files = sorted([f for f in os.listdir(images_dir) if f.endswith('.png')])
    train_files, test_files = train_test_split(all_files, test_size=args.test_split_ratio, random_state=42)

    def move_files(file_list, destination_dir):
        for fname in file_list:
            shutil.move(images_dir / fname, destination_dir / "images" / fname)
            shutil.move(masks_dir / fname, destination_dir / "masks" / fname)
            if has_fft and (ffts_dir / fname).exists():
                shutil.move(ffts_dir / fname, destination_dir / "ffts" / fname)

    move_files(train_files, train_dir)
    move_files(test_files, test_dir)
    
    # Clean up the now-empty temporary directories
    os.rmdir(images_dir)
    os.rmdir(masks_dir)
    if has_fft:
        os.rmdir(ffts_dir)
    ...
    print(f"Dataset is now ready in: {out_root}")
